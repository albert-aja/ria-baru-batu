...
<div class="box box-blue">
    ...
    <form class="form-horizontal" id="Form" enctype="multipart/form-data">
        @csrf
        <div class="box-body">
            {!! FormHelp::select(1, 'config_roles', 'name', array('name'), '$0$', 'Level Akses', 'role', 'Level Akses', $data->getRoleNames()->first(), 'name != "Owner" AND name != "Owner"') !!}
            {!! FormHelp::input_text(1, 'Nama', 'nama', 'Nama', $data->name) !!}
            {!! FormHelp::input_text(1, 'No. Telepon', 'tlp', 'No. Telepon', $data->tlp) !!}
            {!! FormHelp::input_text(1, 'Email', 'email', 'Email', $data->email) !!}
            {!! FormHelp::input_text(1, 'Username', 'username', 'Username', $data->username) !!}
        ...
        {!! FormHelp::form_footer(route('Owner Kepegawaian Pegawai')) !!}
    </form>
</div>
@endsection

@push('breadcrumbs')
    {{ Breadcrumbs::render('Owner Kepegawaian Pegawai Edit', $data->id) }}
@endpush

@push('page_scripts')
    @include('js.owner.kepegawaian.pegawai.edit')
@endpush
